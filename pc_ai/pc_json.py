            ...
            if len(new_res) > 0:
                if new_res != old_res:
                    ...
                    send_text = (f'{res_data}这是jnd28最新100期数开奖，'
                                 f'请根据个位十位百位号码走势图，利用走势分析法、或者每位的折线图进行预测下一期开杂六或者和值为大或者为小的概率有多大（杂六为三位数字都不同也不是顺数）'
                                 f'并且结合和值开奖走势分析，综合评断下期杂六的概率有多大，不要以单纯杂六的出现次数分析，要结合个位十位百位的走势分析，给出预测杂六与和值是大是小的概率百分比！并且用中文回答')
